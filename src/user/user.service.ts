...
export class UserService {
    ...
    async create(createUserDto: CreateUserDto) {
        ...
        if(data)
            ...
            throw new BadRequestException('Errorin inserting data.');

    }

    async create_folio(createUserDto: CreateUserDto) {
        const user = new this.folioModel(createUserDto);
        const data = await user.save();
        console.log(data);
        if(data)
            return {status:"ok"};
   ...
   async update_to(createUserDto: CreateUserDto,req1){
    //console.log(req1.body.email,"error in guiding==============",createUserDto.first_name,"================");
         const filter = { first_name: createUserDto.first_name };
         await this.userModel.findOneAndUpdate({email:req1.body.email},createUserDto,{new: true});
         var doc = await this.userModel.findOne({email:req1.body.email});
         if(doc)
            return {status:"ok"};
         else
            throw new BadRequestException('Error in updating data.');
    ...
    async findAll(req1) {
       // console.log(req1.query.email);
        const user = await this.userModel.findOne({email : req1.query.email});
        if (!user) {
    ...
    private async saveForgotPassword(req: Request, createForgotPasswordDto: CreateForgotPasswordDto) {
        const forgotPassword = await this.forgotPasswordModel.create({
          ...
          .mailerService
          .sendMail({
            to: createForgotPasswordDto.email, // list of receivers
            from: 'noreply@nest.com', // sender address
            subject: 'Forgot password Module ✔', // Subject line
            text: 'welcome Your recovered password is '+forgotPassword._id, // plaintext body
            html: '<b>welcome your password is </b>'+forgotPassword._id, // HTML body content
          })        
